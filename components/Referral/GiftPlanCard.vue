        ...
        <div class="flex flex-col rounded-lg">
          <div>
            ...
            <i18n class="font-bold text-xl md:text-3xl mt-3">
              Gift a 1-month subscription of
            </i18n>&nbsp;&nbsp;
            <em class="font-bold text-xl md:text-3xl mt-3 text-super"><i18n>QUIZIZZ PREMIUM')</i18n></em>
          </div>
          <div class="w-11/12 mt-6 mb-4">
            <span>
              <i18n class="text-dark-4 text-xs md:text-sm mt-2">As a thank you for being a paying subscriber to Quizizz,
                you now have </i18n>
            </span><span class="font-bold text-sm ml-1 text-dark-3">
              <i18n> three FREE 1-month gift subscriptions </i18n>
            </span> <span class="text-dark-4 text-xs md:text-sm mt-2">
              <i18n>(usual value $36) to gift to your colleagues.</i18n>
            </span>
          </div>
          <div
            v-if="!giftingLimitExhausted"
            class="w-11/12 mb-4"
          >
            <span class="text-dark-4 text-xs md:text-sm"><i18n>Know someone at </i18n></span>&nbsp;
            <span class="font-bold text-sm">{{ getOrgName }}</span>&nbsp;
            <span class="text-dark-4 text-xs md:text-sm"><i18n> who might love Quizizz?</i18n></span>
          </div>
        </div>
        <div v-if="!giftingLimitExhausted">
...
export default {
  ...
  computed: {
    ...
    giftingLimitExhausted() {
      return this.emailIds?.length >= this.$constants.MAX_INDI_PLAN_REFER;
    },
  },
  ...
  methods: {
    ...
    async getLink() {
      if (!this.referralLink) {
        this.isLoading = true;
        const Link = await this.getLinkForRefer();
    ...
    async handleShareInvite() {
      ...
      if (this.emailsToShare?.length > (3 - this.emailIds?.length)) {
      ...
      const response = await this.getLinkForRefer(this.emailsToShare);
      if (response.success) {
        this.showError = false;
    ...
    async getCampaignList() {
      ...
      this.showInput = this.emailIds?.length < 3;
    },
