...
export default {
  ...
  computed: {
    ...
    showSidebardCard() {
      return this.$user.isLoggedIn && this.$featureFlags.isEnabled(this.$constants.FeatureFlagsTypes.SUPER_REFER_US_V1) && !this.$user.hasActiveSnDPlan && this.$user.trialDaysLeft === 0 && this.getCount > 0;
    },
  ...
  methods: {
    ...
    async getCampaignList() {
      ...
      let referredEmail = [];
      const currentCampaign = campaigns?.filter((cmp) => cmp.id === '32748');
