...
export default {
  ...
  methods: {
    computeTimeToReshow() {
      ...
      this.showBanner = lastShowEpoch && (diffHours < this.$constants.CHURN_NOTIFICATION_RESHOW_TIME);
    },
    ...
    async onUpdatePaymentInfo() {
      ...
      this.$analytics.logEvent(this.$webEvents.MANAGE_BILLING_INFO, {
        source: this.$webEvents.CHURN_BANNER_PAYMENT_INFO_CLICK,
        userId: this.$user.id,
        bannerType: this.renderBanner1 ? 'banner1' : 'banner2',
        subscriptionEndAt: this.$user?.subscription?.endAt || null,
      });
