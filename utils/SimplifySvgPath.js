...
class PathFitter {
  ...
  _fitCubic(segments, error, first, last, tan1, tan2) {
    ...
    if (last - first === 1) {
      const pt1 = points[first]; const pt2 = points[last]; const dist = pt1._getDistance(pt2) / 3;
    ...
    let maxError = Math.max(error, error * error); let split; let parametersInOrder = true;
    // Try 4 iterations
    for (let i = 0; i <= 4; i++) {
  ...
  _generateBezier(first, last, uPrime, tan1, tan2) {
    const epsilon = /* #= */ EPSILON; const { abs } = Math; const points = this._points; const pt1 = points[first]; const pt2 = points[last];
    ...
    for (let i = 0, l = last - first + 1; i < l; i++) {
      const u = uPrime[i]; const t = 1 - u; const b = 3 * u * t; const b0 = t * t * t; const b1 = b * t; const b2 = b * u; const b3 = u * u * u; const a1 = tan1._normalize(b1); const a2 = tan2._normalize(b2); const tmp = points[first + i]._subtract(pt1._multiply(b0 + b1))._subtract(pt2._multiply(b2 + b3));
      C[0][0] += a1._dot(a1);
      C[0][1] += a1._dot(a2);
      // C[1][0] += a1.dot(a2);
      C[1][0] = C[0][1];
    ...
    if (abs(detC0C1) > epsilon) {
      // Kramer's rule
      const detC0X = C[0][0] * X[1] - C[1][0] * X[0]; const detXC1 = X[0] * C[1][1] - X[1] * C[0][1];
      // Derive alpha values
      alpha1 = detXC1 / detC0C1;
      alpha2 = detC0X / detC0C1;
    } else {
      // Matrix is under-determined, try assuming alpha1 == alpha2
      const c0 = C[0][0] + C[0][1]; const c1 = C[1][0] + C[1][1];
      alpha1 = alpha2 = abs(c0) > epsilon ? X[0] / c0 : abs(c1) > epsilon ? X[1] / c1 : 0;
    }
    // If alpha negative, use the Wu/Barsky heuristic (see text)
    // (if alpha is 0, you get coincident control points that lead to
    // divide by zero in any subsequent NewtonRaphsonRootFind() call.
    const segLength = pt2._getDistance(pt1); const eps = epsilon * segLength;
    let handle1; let
      handle2;
  ...
  _findRoot(curve, point, u) {
    ...
    const pt = this._evaluate(3, curve, u); const pt1 = this._evaluate(2, curve1, u); const pt2 = this._evaluate(1, curve2, u); const diff = pt._subtract(point); const df = pt1._dot(pt1) + diff._dot(pt2);
    // u = u - f(u) / f'(u)
  ...
  _findMaxError(first, last, curve, u) {
    let index = Math.floor((last - first + 1) / 2); let maxDist = 0;
    for (let i = first + 1; i < last; i++) {
...
const getSegmentsPathData = (segments, closed, precision) => {
  ...
  let first = true;
  let prevX; let prevY; let outX; let
    outY;
  const parts = [];
